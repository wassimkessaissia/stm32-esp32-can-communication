...
int CANSPI_Initialize(void)
{
  ...
  /* Rx Mask values - ALL ZEROS = Accept all */
  ...
  MCP2515_WriteByteSequence(MCP2515_RXF1SIDH, MCP2515_RXF1EID0, &(RXF1reg.RXF1SIDL));
  ...
  MCP2515_WriteByte(MCP2515_RXB0CTRL, 0x60);//(bit 6-5  = 11)
  MCP2515_WriteByte(MCP2515_RXB1CTRL, 0x60);//(bit 6-5  = 11)

  /*
   * CAN Bit Timing for 500kbps @ 8MHz crystal
   * TQ = 2 * (BRP + 1) / FOSC = 2 * (0 + 1) / 8000000 = 0.25us
   * Bit Time = (SYNC_SEG + PROP_SEG + PS1 + PS2) * TQ
   * Bit Time = (1 + 1 + 2 + 3) * 0.25us = 1.75us ≈ 2us = 500kbps
   * Actually: (1 + 1 + 3 + 3) = 8 TQ * 0.25us = 2us = 500kbps
   */
  MCP2515_WriteByte(MCP2515_CNF1, 0x00);  // SJW=1TQ, BRP=0
  MCP2515_WriteByte(MCP2515_CNF2, 0x90);  // BTLMODE=1, PHSEG1=2TQ, PRSEG=1TQ
  MCP2515_WriteByte(MCP2515_CNF3, 0x02);  // PHSEG2=3TQ
